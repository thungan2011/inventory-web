...
export interface ExportProductLocation {
    id: number;
...
type ModalChooseExportLocationProps = {
    ...
    onSubmit: (locations: ExportProductLocation[]) => void;
    selectedLocations: ExportProductLocation[];
    productCode: string;
}

interface FormValues {
    exportLocations: ExportProductLocation[];
}
...
const FormikContent = ({ onClose, totalQuantity, productCode }: FormikContentProps) => {
    ...
    const storageAreaQuery = useAllWarehouseAreaProducts({
    ...
    const handleAddStorageArea = (item: WarehouseAreaProductOverview) => {
        const isExist = values.exportLocations.find(a => a.id === item.id);
        if (!isExist) {
            const newLocation: ExportProductLocation = {
                id: item.id,
...
const ModalChooseExportProductLocation = ({
                                              onClose,
                                              totalExportQuantity,
                                              onSubmit,
                                              selectedLocations,
        ...
        <>
            <Modal title="Phân bổ số lượng xuất kho" open={true} onClose={onClose}>
                <Formik initialValues={initialValues} onSubmit={handleSubmit}
                    ...
                    <FormikContent onClose={onClose} totalQuantity={totalExportQuantity} productCode={productCode} />
                </Formik>
            </Modal>
        </>
    );
...
export default ModalChooseExportProductLocation;